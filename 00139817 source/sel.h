...
Matrix createLocalK(int e,mesh &m){
    ...
    float t, k, l, d, eta, Ae, J, D;
    
    //Componentes de K
    // [ A+K   D-C ]
    // [  E    0  ]

    Matrix matrixA,matrixK,matrixD,matrixC,matrixE;
    ...
    //PREPARANDO MATRIZ D
    l = m.getParameter(Lam);
    productRealMatrix((2*J*l)/(18*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixD);
    
    //PREPARANDO MATRIZ A
    t= m.getParameter(Tau);
    ...
    //PREPARANDO MATRIZ C
    d = m.getParameter(Delt);
    productRealMatrix((-1)*(d*Ae)/(8*D*D),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,BPrima,2,2,3),2,2,3),6,2,3),matrixC);

    //PREPARANDO MATRIZ K
    k = m.getParameter(Kap);
    ...
    ubicarSubMatriz(K,0,5,6,8,sumMatrix(matrixD,matrixC,6,3));
...
Vector createLocalb(int e,mesh &m){
    ...
    float i = m.getParameter(Ips);
    float p = m.getParameter(Psi);
